...
class QiniuStorage extends StorageBase {
  ...
  constructor (options) {
    ...
    const { bucket, accessKey, secretKey, domain, format = '${years}/${months}/${name}${ext}' } = options
   ...
   * @private
   */
  getDirectory (original) {
    const context = getPathContext(original)
   ...
   */
  save (file, targetDir) {
    targetDir = targetDir || this.getDirectory(file.name)
  ...
  read (file) {
    ...
    return got.get(uri, options).then(res => res.body)
  }
  ...
  exists (filename, targetDir) {
    ...
    return got.head(uri, options).then(() => true).catch(() => false)
  }
